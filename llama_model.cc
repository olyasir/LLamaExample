...
void  callback_func(Array<RequestStreamOutput> out)
{
 ...
 std::string LlamaModel::Process(const std::string prompt) {
         ...
         GetEngine()->SetRequestStreamCallback(callback_func);
         ...
         std::string generation_cfg_json_str = "{\"n\":1,\"max_tokens\":1024,\"stop_token_ids\":[2]}";

         auto config = json::ParseToJSONObject(generation_cfg_json_str);
         auto gen_config = GenerationConfig::FromJSON(config, _generation_config);

         GenerationConfig g_config = gen_config.Unwrap();
